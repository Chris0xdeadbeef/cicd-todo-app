...
watch(() => props.todoCompleted, (newVal) => {
...
const deleteTodoItem = async () => {
  ...
  todoStore.allTodo = todoStore.allTodo.filter(todo => todo.id !== props.todoId);
};
</script>
...
<template>
  ...
  >
    <div class="flex items-center justify-between w-full border-b border-gray-300 dark:border-gray-600 pb-2">
      <div class="flex items-center">
        <span class="w-[60px] dark:text-gray-200">Pour le :</span>
        <span class="text-sm text-gray-700 dark:text-white font-bold">
          {{ new Date(todoDate).toLocaleDateString('fr-FR', { year: 'numeric', month: 'long', day: 'numeric' }) }}
        </span>
      </div>
      <div class="flex flex-1 align-middle justify-end">
        <Toggle
          ...
          @change="toggleTodo"
          :classes="{ 
            container: 'inline-block w-[70px] rounded-full outline-none focus:ring-2 focus:ring-slate-400 focus:ring-opacity-30',
            toggle: 'flex w-[68px] h-5 rounded-full relative cursor-pointer transition items-center box-content border-1 text-xs leading-none',
            ...
            handleOff: 'left-0',
            label: 'text-gray-600 dark:text-gray-700 text-center font-bold w-8 border-box whitespace-nowrap select-none pl-1 pr-1 ml-[8px] mr-[15px]'
          }"
      ...
      <div class="ml-4 flex-shrink-0">
        <button @click="deleteTodoItem">
          <TrashIcon class="h-5 w-5 stroke-gray-600 dark:stroke-white hover:stroke-red-500 cursor-pointer" />
        </button>
      </div>
    </div>
    <div class="flex w-full flex-1 bg-white dark:bg-gray-200 mt-[10px] ml-[10px] mr-[10px] mb-[0px] rounded-[4px]">
